...
from .router import TaskRouter, TaskAnalysis, ExecutionStrategy, ExecutionPlan
from .memory import MemoryManager
from .council.executor import CouncilExecutor, CouncilExecutionRequest
...
class TaskOrchestrator:
    ...
    def __init__(
        ...
        # Bug fix 1.1: Add lock for thread-safe access to active_tasks
    ...
    async def submit_task(
        ...
        # Bug fix 1.1: Add task with lock protection
    ...
    async def _orchestrate_task(self, context: TaskContext) -> None:
        ...
        try:
            ...
            execution_plan = self.task_router.route(
                prompt=context.prompt,
                context=context.workspace_context,
            )

            # Check if task should use council deliberation
            if execution_plan.strategy == ExecutionStrategy.COUNCIL:
                await self._execute_council(context, execution_plan)
                return
    ...
    async def _execute_council(
        ...
        """Execute task using council deliberation.

        Council strategy runs multiple perspectives to analyze and
        synthesize a solution for high-risk or complex tasks.

        Args:
            context: Task context
            execution_plan: Execution plan with council perspectives
        """
        if not self.council_executor:
            logger.warning(
                f"Council strategy requested for {context.task_id} but no "
                "council executor available. Falling back to standard execution."
            )
            # Fall back to standard execution
            plan = await self._compile_plan(context, context.analysis)
            context.plan = plan
            context.state = TaskState.RUNNING
            context.started_at = datetime.utcnow()
            await self._update_task_state(context)
        ...
        logger.info(
            f"Executing council deliberation for {context.task_id} "
            f"with perspectives: {execution_plan.council_perspectives}"
        )

        # Build council execution request
        request = CouncilExecutionRequest(
            task_id=context.task_id,
            session_id=context.session_id,
            user_id=context.user_id,
            task_description=context.prompt,
            proposed_code="",  # Code will be generated by council
            context=context.workspace_context,
            analysis=context.analysis,
            council_perspectives=execution_plan.council_perspectives,
        )

        # Execute council deliberation
        result = await self.council_executor.execute(request)

        # Emit council result event
        await self._emit('council_result', context, result)

        if result.approved:
            # Council approved - proceed with execution
            plan = await self._compile_plan(context, context.analysis)
            context.plan = plan
            context.state = TaskState.RUNNING
            context.started_at = datetime.utcnow()
            ...
            # Council blocked the task
            logger.warning(
                f"Council blocked task {context.task_id}: "
                f"{result.blocking_reasons}"
            )
            await self._emit('council_blocked', context, result)
            context.state = TaskState.WAITING_APPROVAL
            context.error_reason = f"Council blocked: {', '.join(result.blocking_reasons)}"
            await self._update_task_state(context)
    ...
    async def _complete_task(self, context: TaskContext) -> None:
        ...
        # Bug fix 1.1: Remove from active tasks with lock
        async with self._tasks_lock:
    ...
    async def handle_client_reconnect(self, session_id: str) -> None:
        ...
        # Bug fix 5.2: Use list() to avoid dictionary modification during iteration
        for task_id, context in list(self.active_tasks.items()):
            if context.session_id == session_id:
                # Resume if paused
